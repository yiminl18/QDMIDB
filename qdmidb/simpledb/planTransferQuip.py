...
path = "/Users/yiminglin/Documents/Codebase/QDMIDB/QDMIDB/qdmidb/queryplancodes/wifi/cdcQueryPlans.txt"
...
for key in new_order_list:
    if 'μ' in key:
        ...
        order_list_noimp.append(key)
        noimp_dict[key]=[]
        sp = key
        for child in tree_dict[key]:
            print(child)
            while 'μ' in child:
                child = tree_dict[child][0]
            noimp_dict[key].append(child)

print(noimp_dict)
print(order_list_noimp)
...
for node in order_list_noimp:
    ...
    if 'μ' in node:
        ...
        childNodeName = name_dict[noimp_dict[node][0]]
    ...
    if '⨝' in node:
        ...
        child1Name = name_dict[noimp_dict[node][0]]
        child2Name = name_dict[noimp_dict[node][1]]
    ...
    if 'g(' in node or 'min(' in node or 'max(' in node or 'avg(' in node or 'sum(' in node or 'count(' in node:
        ...
        groupChildName = name_dict[noimp_dict[node][0]]
